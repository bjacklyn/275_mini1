...
void Collisions::combine(const Collisions& other) {
    ...
    /*
    // Merge buckets2 into buckets1
    for (const auto& entry : other.buckets) {
        // If the key is already in buckets1, append the vector
        if (buckets.find(entry.first) != buckets.end()) {
            buckets[entry.first].insert(buckets[entry.first].end(), entry.second.begin(), entry.second.end());
        } else {
            // Otherwise, just add the key-value pair
            buckets[entry.first] = entry.second;
        }
    }
    */
